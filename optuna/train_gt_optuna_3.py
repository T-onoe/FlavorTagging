...
class NodeAndGraphClassification(torch.nn.Module):
    def __init__(self, fc1, fc2, fc3, fc4, fc5, sg1, sg2, sg3):
        super(NodeAndGraphClassification, self).__init__()
        self.fc1 = nn.Linear(5, fc1)
        self.sage1 = SAGEConv(fc1, sg1)
        self.bn1 = BatchNorm(sg1)
        self.sage2 = SAGEConv(sg1, sg2)
        self.bn2 = BatchNorm(sg2)
        self.sage3 = SAGEConv(sg2, sg3)
        self.bn3 = BatchNorm(sg3)
        self.fc2 = nn.Linear(sg3, fc2)
...
def objective(trial):
    ...
    fc4 = trial.suggest_int("fc4", 16, 1024)
    fc5 = trial.suggest_int("fc5", 16, 1024)
    sg1 = trial.suggest_int("sg1", 16, 1024)
    sg2 = trial.suggest_int("sg2", 16, 1024)
    sg3 = trial.suggest_int("sg3", 16, 1024)
    lr = trial.suggest_float("lr", 0.00001, 0.1)
    alpha = trial.suggest_float("alpha", 0.5, 2.0)
#    num_epochs = trial.suggest_int("num_epochs", 10, 100)

    model = NodeAndGraphClassification(fc1, fc2, fc3, fc4, fc5, sg1, sg2, sg3).to(device)
...
#    val_loss = 0
    ...
    def train():
        ...
        for data in train_loader:
            ...
            loss = combined_loss(alpha, node_out, graph_out, data.node_y, data.y)
            loss = loss.to(device)
    ...
    for epoch in range(50):
        train_loss, train_graph_loss = train()
    val_loss, val_graph_loss = validation()
